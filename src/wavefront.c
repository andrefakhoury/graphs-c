...
void graph_waveFront(Graph* g, int x, int y, Error* error) {
	...
	while(!empty(q)) {
        ...
        /* The east values of wavefront are setted */
		if ((atX+1) < GRID && *((int*)graph_edgeWeight(g, atX+1, atY, error)) == -1) {
			graph_addEdge(g, atX+1, atY, &cnt, error);
			push(q, atX+1, atY);
		}
		...
		if ((atY+1) < GRID && *((int*)graph_edgeWeight(g, atX, atY+1, error)) == -1) {
			graph_addEdge(g, atX, atY+1, &cnt, error);
			push(q, atX, atY+1);
		}
	}
...
void graph_walk(Graph* g, int px, int py, int gx, int gy, Error* error) {    
    ...
    while (px != gx || py != gy) { /* While Pac Man does not reach the ghost */
        ...
        else if (ghost == 2 && gy+1 < GRID && (px != gx && py != gy))
            gy++, flag = 1;
        /* The ghost goes to east */
        else if (ghost == 3 && gx+1 < GRID && (px != gx && py != gy))
            gx++, flag = 1;
        ...
        /* Pac Man goes to east */
        if (px+1 < GRID && *((int*)graph_edgeWeight(g, px+1, py, error)) < min) {
        ...
        if (py+1 < GRID && *((int*)graph_edgeWeight(g, px, py+1, error)) < min) {
